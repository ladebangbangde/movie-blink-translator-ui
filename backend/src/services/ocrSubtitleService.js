...
function buildSegmentsFromFrames(frames, intervalSec, options) {
  const minStableFrames = options.minStableFrames || Math.max(2, Math.round(0.35 / intervalSec));
  const maxGapFrames = options.maxGapFrames || Math.max(1, Math.round(0.25 / intervalSec));
  ...
  for (let i = 0; i < frames.length; i += 1) {
    ...
    if (!current) {
      current = {
        text,
        startSec: item.timestampSec,
        endSec: item.timestampSec + intervalSec,
        frameCount: 1,
        gapFrames: 0
      };
      continue;
    ...
    if (sim >= 0.82) {
      current.endSec = item.timestampSec + intervalSec;
...
function withTimestamps(frameTexts, intervalSec) {
  return frameTexts.map((item, index) => ({
    text: item.text || '',
    confidence: Number.isFinite(item.confidence) ? item.confidence : 0,
    timestampSec: Number((index * intervalSec).toFixed(3))
  }));
}
...
export async function extractHardSubtitleWithOcr(inputPath, outputPath, options = {}) {
  ...
  try {
    await runCommand('ffmpeg', [
      '-y',
      '-i', inputPath,
      '-vf', `fps=${1 / intervalSec},${cropExpr},scale=1920:-1,format=gray,eq=contrast=1.45:brightness=0.03,unsharp=7:7:1.2:7:7:0.0`,
      '-q:v', '2',
      framePattern
    ]);
    ...
    const timedFrameTexts = withTimestamps(frameTexts, intervalSec);
    const smoothed = smoothFrameTexts(timedFrameTexts);
    const segments = buildSegmentsFromFrames(smoothed, intervalSec, {
    ...
    const timelineDebugPath = `${outputPath}.timeline.json`;
    fs.writeFileSync(timelineDebugPath, JSON.stringify({
      intervalSec,
      frameCount: timedFrameTexts.length,
      recognizedFrames: timedFrameTexts.filter((it) => it.text).length,
      segments
    }, null, 2), 'utf-8');
  } finally {
