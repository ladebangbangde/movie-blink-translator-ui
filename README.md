...
### 8) 按你的需求：不新开分支，直接在当前分支提交并发布

如果你希望「本地 IDEA 开发 -> 提交到当前分支 -> 发布到远端 Docker」，可以按下面最小流程执行。
...
#### 8.2 构建并推送镜像（可在本地或 CI）

```bash
docker login your-registry
docker build -f backend/Dockerfile -t your-registry/movie-blink-backend:vX.Y.Z .
docker build -f frontend/Dockerfile -t your-registry/movie-blink-frontend:vX.Y.Z .
docker push your-registry/movie-blink-backend:vX.Y.Z
docker push your-registry/movie-blink-frontend:vX.Y.Z
```

#### 8.3 远端更新并生效

在服务器把 `docker-compose.prod.yml` 中镜像 tag 改为 `vX.Y.Z`，然后执行：
